...
In the chunk below, I ran a regression and modeled it with ggplot. Due to the fact that my dependent and independent variable are categorical, I have coded them to be numerical with the mutate function which results in categorical binary independent and dependent variables. 1 for elected, 0 for not elected, 1 for fully denied the election results of 2020 and 0 for accepted the results of the 2020 election. Due to the fact that my dependent variable is not continuous but rather binary I could not conduct a linear regression on this data so I did some research and found that a logistic regression model would be better suited for my data. I used to this website (https://www.statology.org/logistic-regression-in-r/) to inform the code I have written in the chunk below. To control for other independent variables in my data set such as the state, incumbency status, office, and what president the state went for in 2020 I added them as variables in my regression model. 
...
regression_fit <- glm(elected ~ stance + President + incumbency + state + office , regression, family = "binomial") 

options(scipen=999)

modelsummary::modelsummary(regression_fit)

library(pscl)
pscl::pR2(regression_fit)["McFadden"]

ggplot(regression, mapping = aes(x = stance, y = elected))  + geom_smooth(method = "glm")
